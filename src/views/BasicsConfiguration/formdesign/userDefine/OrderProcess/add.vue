  ...
  <div class="flex-column">
    <sticky :className="'sub-navbar'">
      <div class="filter-container">
        ...
        <permission-btn moduleName='orderProcessTbls' size="mini" v-on:btn-event="onBtnClicked"></permission-btn>
      </div>
    </sticky>
...
<script>
  import * as orderProcessTbls from '@/api/orderprocesstbls'
  import * as orderProcessDtbls from '@/api/orderprocessdtbls'
  ...
  export default {
    ...
    data() {
      return {
        ...
        editModel: true, // 是否为编辑模式
        editType: 'add', // 编辑类型
        dialogStatus: 'create', // 主修改对话框状态create/update
        selectRow: {},
    ...
    methods: {
      handleFilterStatus(val, key) {
        ...
        return key === 'asnStatus'  || key =='inStockStatus' ? val === 0 ? '否' : '是' : arr.find(item => item.key === val).display_name
      },
      ...
      getList() {
        ...
        orderProcessTbls.getList(this.firstQuery).then(response => {
      ...
      createData() { // 保存提交
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            orderProcessTbls.add(tempData).then(() => {
      ...
      setDetails(tempData) { // 处理明细
        ...
        this.secondList.length > 0 && this.secondList.forEach(item => {
          ...
          tempData.orderProcessDtblReqs.push(obj)
        })
        return tempData
      },
      updateData() { // 更新提交
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            let tempData = Object.assign({}, this.firstTemp)
            tempData = this.setDetails(tempData)
            orderProcessTbls.update(tempData).then(() => {
      ...
      handleFirstDel(row) { // 删除头
        orderProcessTbls.del([row.id]).then(() => {
          this.$notify({
            title: '成功',
            message: '删除成功',
            type: 'success',
